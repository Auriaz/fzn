...
class ArticleReferenceAdminController extends BaseController
{
    ...
    public function uploadArticleReference(Article $article, Request $request, UploaderHelper $uploaderHelper,  EntityManagerInterface $entityManager, ValidatorInterface $validator)
    {
        ...
        // In MimeTypeExtensionGuesser.php i find meny difrend extension 
        $violations = $validator->validate(
            $uploadedFile,
            [
                new NotNull([
                    'message' => 'Nie wybrano pliku! Wybierz plik!',
                ]),
                new File([
                    'maxSize' => '5M',
                    'maxSizeMessage' => 'Dany plik jest za duży ({{ size }} {{ suffix }}). Maxymalna wielkość to {{ limit }} {{ suffix }}.',
                    'mimeTypes' => [
                        'image/*',
                        'application/pdf',
                        'application/msword',
                    ...
                    'mimeTypesMessage' => 'Rozszeżenie danego pliku nie jest zgodne!'
                ]),
            ]
        );
